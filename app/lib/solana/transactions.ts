...
import { getSolanaConnection, isTokenPurchaseTransaction, estimateTransactionSolValue, extractTokenSymbol, getTokenMarketCap } from './connection';

// Real token data - in a production app, this would come from an API or token registry
const tokenData: Record<string, TokenInfo> = {
  'POPCAT': { symbol: 'POPCAT', name: 'Pop Cat Token', marketCap: 1220000, price: 0.05 },
  'BONK': { symbol: 'BONK', name: 'Bonk Token', marketCap: 450000000, price: 0.00002 },
  'SAMO': { symbol: 'SAMO', name: 'Samoyedcoin', marketCap: 120000000, price: 0.03 },
  'MANGO': { symbol: 'MANGO', name: 'Mango Markets', marketCap: 75000000, price: 0.15 },
  'COPE': { symbol: 'COPE', name: 'Cope Token', marketCap: 25000000, price: 0.25 },
};
...
export const streamTransactions = (
  ...
  const processTransaction = async (signature: string, blockTime?: number) => {
    try {
      ...
      if (filter?.minMarketCap && (!marketCap || marketCap < filter.minMarketCap)) {
        return;
      }
      
      if (filter?.maxMarketCap && (marketCap && marketCap > filter.maxMarketCap)) {
        return;
      }
      ...
      const transaction: SolanaTransaction = {
        id: signature,
        walletAddress: tx.transaction.message.accountKeys[0].pubkey.toString(),
        tokenSymbol,
  ...
  const pollForTransactions = async () => {
    try {
      ...
      for (const sig of signatures) {
        if (isStreaming) {
          await processTransaction(sig.signature, sig.blockTime ?? undefined);
        }
      }
  ...
  const shouldIncludeTransaction = (tx: SolanaTransaction): boolean => {
    ...
    if (filter.minMarketCap && (!tx.marketCap || tx.marketCap < filter.minMarketCap)) return false;
    if (filter.maxMarketCap && (tx.marketCap && tx.marketCap > filter.maxMarketCap)) return false;
    if (filter.tokenSymbol && tx.tokenSymbol !== filter.tokenSymbol) return false;
