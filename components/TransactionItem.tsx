...
import { formatWalletAddress, getSolscanUrl } from '../app/lib/solana/connection';
...
export default function TransactionItem({ transaction, onWalletClick }: TransactionItemProps) {
  const { walletAddress, tokenSymbol, amountInSol, timestamp, txSignature } = transaction;
    ...
    <>
      <div 
        ...
        className={`group p-2.5 bg-gray-900 rounded-2xl hover:bg-gray-800/50 transition-all duration-2000 mb-3 w-[75%] mx-auto ${
          showGlow ? 'border border-green-500/40 shadow-[0_0_25px_rgba(74,124,89,0.5)]' : ''
        }`} 
      >
        <div className="flex items-start justify-between gap-2.5">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-base font-semibold text-white">{amountInSol} SOL</span>
              ...
              {transaction.marketCap && (
                <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-gray-500/10 text-gray-400 text-xs">
                  {(transaction.marketCap / 1000000).toFixed(1)}M
                </span>
              )}
            </div>
      ...
      {showWalletDetails && (
        <div className="fixed right-0 top-0 h-full w-80 bg-gray-900 border-l border-gray-800 shadow-lg overflow-y-auto z-50 transition-transform duration-300">
          ...
          <div className="p-4">
            <div className="mb-6">
              <h4 className="text-sm font-medium mb-3 text-gray-300">Balance</h4>
            ...
            <div className="mb-6">
              <h4 className="text-sm font-medium mb-3 text-gray-300">Tokens</h4>
            ...
            <div>
              <h4 className="text-sm font-medium mb-3 text-gray-300">Recent Transactions</h4>
              <div className="space-y-3">
                <div className="py-2 border-b border-gray-800 last:border-0">
                  <div className="flex justify-between items-center">
                    <div>
                      <span className="px-1.5 py-0.5 rounded-full bg-green-500/10 text-green-400 text-xs font-medium">${tokenSymbol}</span>
                      <span className="text-gray-300 ml-2 text-sm">{amountInSol} SOL</span>
                    </div>
                    <a 
                      href={getSolscanUrl(txSignature, 'tx')} 
                      target="_blank" 
